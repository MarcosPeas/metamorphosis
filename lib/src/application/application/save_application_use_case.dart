...
class SaveApplicationUseCase {
  ...
  SaveApplicationUseCase({required ApplicationRepository applicationRepository}) {
    _applicationRepository = applicationRepository;
  }

  Future<Either<DomainException, Application>> execute(Application application) async {
    try {
      final result = await _applicationRepository.save(application);
      return Right(result);
      ...
      return Left(
        DomainException.of(
          message: e.toString(),
          trace: s.toString(),
          context: 'SaveApplicationUseCase',
        ),
      );
    }
  }
