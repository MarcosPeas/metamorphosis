...
class ValueObjectRuleRepositoryImpl implements ValueObjectRuleRepository {
  ...
  @override
  Future<List<ValueObjectRule>> filter(PaginateParams params) async {
    try {
  ...
  @override
  Future<void> delete(ValueObjectRule vor) async {
    try {
      _firestore.collection(collection).doc(vor.id).delete();
    } on auth.FirebaseAuthException catch (e, s) {
      throw DomainException.of(
        message: e.code,
        trace: s.toString(),
  ...
  @override
  Future<Page<ValueObjectRule>> paginate(Filter filter) {
